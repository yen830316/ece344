...
// debug flags global variable
static char dbflags_val = 0;
...
cmd_dbflags(int nargs, char **args)
{
    ...
    int bit_num = atoi(args[1]);
    if (bit_num < 1 || bit_num > 12) {
		kprintf("Usage: df nr on/off\n");
		return EINVAL;
    }
    ...
    char masker; 
    if (!strcmp(args[2], "on")) {
        masker = 0x0001 << (bit_num - 1);
        dbflags_val = dbflags_val | masker;
    } else if (!strcmp(args[2], "off")) {
        masker = 0xfffe << (bit_num - 1);
        dbflags_val = dbflags_val & masker;
    } else {
		kprintf("Usage: df nr on/off\n");
		return EINVAL;
